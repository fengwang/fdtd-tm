/* 
 * File:   dynamic_matrix_buffer.hpp
 * Author: feng
 *
...
/*
 *
 *      This class privides memory allocation and deallocation for dynamic_matrix.
 ...
 *      size determined during runtime. dynamic_matrix_buffer maintains a fixed
...
template<
            ...
            std::size_t SPACE = 1024,               // default stack size used
            class Allocator = std::allocator<Type>  // fall-back heap allocator
        >
class dynamic_matrix_buffer : public Allocator
{
public:
    typedef Type                                    value_type;
    typedef dynamic_matrix_buffer                   self_type;
...
public:
    ...
    //
    explicit dynamic_matrix_buffer(const size_type dims = 0)
    {
        items_ = dims;
    ...
    dynamic_matrix_buffer(Input_Iterator begin_, Input_Iterator end_)
    {
...
public:
    ...
    dynamic_matrix_buffer(const dynamic_matrix_buffer<T,D,A>& rhs)
    {
        operator = <T,D,A> (rhs);
    }
    ...
    template< typename T, std::size_t D, typename A >
    self_type &
    operator=(const dynamic_matrix_buffer<T,D,A>& rhs)
    {
        do_copy<T,D,A> (rhs);
        return *this;
    }
...
public:
    ...
    // Description:
    //              test if dynamic_matrix_buffer holds any elements
    // Returns:
    ...
    // Description:
    // Return:
    //              elements stored in dynamic_matrix_buffer
    //
    size_type 
    size()const
    {
...
public:
    ...
    // Description:
    // Return:      the first element position stroed in dynamic_matrix_buffer
    //
    iterator 
    begin()
    {
    ...
    // Description:
    // Return:      the first position following the stroed in dynamic_matrix_buffer
    //
    iterator 
    end()
    {
...
public:
    ...
    // Description :
    //                  lvalue of elements stored in dynamic_matrix_buffer
    //
    reference 
    ...
    // Description :
    //                  const lvalue of elements stored in dynamic_matrix_buffer
    //
    const_reference 
...
private:
    ...
    //
    template<typename T, std::size_t D, typename A>
    void do_copy(const dynamic_matrix_buffer<T,D,A>& rhs)
    {
...
public: 
    ...
    //                  ctor
    //
    dynamic_matrix_buffer(const self_type& rhs)
    : Allocator(rhs) 
    {
        operator = (rhs);
    }
